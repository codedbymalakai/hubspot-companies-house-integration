...
const searchContactsAPI = "https://api.hubapi.com/crm/v3/objects/contacts/search";
...
exports.main = async (context = {}) => {
    ...
    const BASE_COMPANIES_HOUSE_URL = "https://api.company-information.service.gov.uk";
    ...
    try {
        ...
        // Officers Info
        const officersResponse = await axios.get(`${BASE_COMPANIES_HOUSE_URL}/company/${companyNumber}/officers`, {
        ...
        const formattedLast = last.trim().toLowerCase().replace(/^\w/, c => c.toUpperCase());
  ...
  if (companyId && created_contacts.length > 0) {
    for (let contactId of created_contacts) {
      ...
      if (!assocResponse.ok) {
        console.error(`Failed to associate contact ${contactId} with company ${companyId}`);
      } else {
